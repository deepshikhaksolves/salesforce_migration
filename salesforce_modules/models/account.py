...
class Account(models.Model):
    ...
    market_reserve_ids = fields.One2many('model_market_reserve__c', 'Account_c', string='Market Reserve')
    account_ids = fields.One2many('account.account', 'Account_c', string='Accounts')
    broker_ids = fields.One2many('broker', 'Account', string='Broker')
    expense_provision_ids = fields.One2many('expence_provisioning', 'Income_id', string='Expense Provisions')
    benefit_carrier_ids = fields.One2many('model_beneficiary_carrier_revenue', 'carrier_id', string='Benefit Carriers Revenue')
    account_pathology_ids = fields.One2many('model_pathology_account_c', 'account_id', string='Account Pathologies')
    operator_checklist_ids = fields.One2many('model_operator_checklist', 'operator_c', string='Operator Checklist')